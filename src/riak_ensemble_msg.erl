...
send_all(Msg, Id, Peers, Views, Required) ->
    ...
    #msgstate{awaiting=ReqId, timer=Timer, replies=[], id=Id, views=Views, required=Required}.
...
send_request({PeerId, PeerPid}, ReqId, Event) ->
    case PeerPid of
        undefined ->
            ...
            _ = lager:info("Sending offline nack for ~p :: ~p", [PeerId, Event]),
...
quorum_met(_Replies, _Id, [], _Required) ->
    %% TODO: Consider if Required > 0 should actually fail here
    true;
quorum_met(Replies, Id, [Members|Views], Required) ->
    ...
    Quorum = case Required of
                 ...
                 other ->
                     ...
                     length(Members) div 2 + 1;
                 all ->
                     _ = lager:info("Quorum: all = ~p", [length(Members)]),
                     _ = lager:info("Filtered: ~p", [Filtered]),
                     length(Members)
             end,
    %% TODO: Use of other is a hack. Fix.
