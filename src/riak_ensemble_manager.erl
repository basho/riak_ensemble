...
-export([start/0,
         ...
         check_quorum/2,
         count_quorum/2]).
-export([check_ensembles/2, check_peers/1]).
...
join(Same, Same) ->
    {error, same_node};
join(Node, OtherNode) ->
    gen_server:call({?MODULE, OtherNode}, {join, Node}, infinity).

-spec enabled() -> boolean().
enabled() ->
    try
        ets:lookup_element(em, enabled, 2)
    catch _:_ ->
            false
    end.
...
cluster() ->
    try
        ets:lookup_element(em, cluster, 2)
    catch _:_ ->
            []
    end.
...
get_remote_peer_pid(Ensemble, PeerId) ->
    try
        ets:lookup_element(em, {remote_pid, {Ensemble, PeerId}}, 2)
    catch
        _:_ ->
            undefined
    end.
...
get_pending(EnsembleId) ->
    try
        ets:lookup_element(em, EnsembleId, 4)
    catch _:_ ->
            undefined
    end.
...
get_members(EnsembleId) ->
    Views = try
                {_, Vs} = ets:lookup_element(em, EnsembleId, 3),
                Vs
            catch _:_ ->
                    []
            end,
...
init([]) ->
    ...
    State = reload_state(),
    schedule_tick(),
    true = ets:insert(em, {cluster_state, State#state.cluster_state}),
...
handle_cast({peer_pid, Peer, Pid}, State=#state{remote_peers=Remote}) ->
    ...
    ets:insert(em, {{remote_pid, Peer}, Pid}),
    erlang:monitor(process, Pid),
    _ = lager:info("Tracking remote peer: ~p :: ~p~n", [Peer, Pid]),
        ...
        _ ->
            case lists:keytake(Pid, 2, Remote) of
                {value, {Peer, _Pid}, Remote2} ->
                    ets:delete(em, {remote_pid, Peer}),
                    _ = lager:info("Untracking remote peer: ~p :: ~p~n", [Peer, Pid]),
...
activate(State=#state{cluster_state=CS}) ->
    case riak_ensemble_state:enabled(CS) of
        ...
        false ->
            ...
            ets:insert(em, {root, RootLeader, element(2, Root), undefined}),
...
compute_all_members(_Ensemble, undefined, Views) ->
    compute_members(Views);
...
state_changed(State=#state{ensemble_data=EnsData, cluster_state=CS}) ->
    true = ets:insert(em, {cluster_state, CS}),
    true = ets:insert(em, {cluster, riak_ensemble_state:members(CS)}),
    true = ets:insert(em, {enabled, riak_ensemble_state:enabled(CS)}),
    {NewEnsData, EnsChanges} = check_ensembles(EnsData, CS),
    _ = [case Change of
             {add, Obj} ->
                 true = ets:insert(em, Obj);
             {del, Obj} ->
                 true = ets:delete(em, Obj);
             {ins, Obj} ->
                 true = ets:insert(em, Obj)
    ...
    _ = [case Change of
             {add, {Ensemble, Id}, Info} ->
                 ...
                 %% Start new peer
                 %% io:format("Should start: ~p~n", [{Ensemble, Id, Mod, Args}]),
...
-spec compute_members([[peer_id()]]) -> [peer_id()].
...
%% ETS table contains three items:
%% -- Ensemble information: {Ensemble, Leader, Views, Pending}
%% -- Peer pids: {{pid, {Ensemble, Id}}, Pid}
%% -- Peer ets: {{ets, {Ensemble, Id}}, tab()}
%%
%% Also have "supervisor" state. Eg. set of tracked children
%%
%% Goal is to make ETS be a reflection of stable state.
%% Ensemble information comes from cluster_state
%% Pids/ETS information comes from riak_ensemble_peer_sup and is set by peers
%%
%% Since ETS is owned by ensemble manager, it is empty if manager ever restarts.
%% So manager should be able to track believed state.
...
-spec ensemble_data(cluster_state())
                   -> orddict:orddict(ensemble_id(),
                                      {ensemble_id(), peer_id(), {vsn(), views()}, {vsn(), views()}}).
ensemble_data(CS) ->
    Ensembles = riak_ensemble_state:ensembles(CS),
    Pending = riak_ensemble_state:pending(CS),
...
-spec wanted_peers(cluster_state()) -> orddict:orddict({ensemble_id(), peer_id()},
                                                       ensemble_info()).
wanted_peers(CS) ->
    Ensembles = riak_ensemble_state:ensembles(CS),
    Pending = riak_ensemble_state:pending(CS),
