...
%% This is an old jtuple joint, and it's not clear what it would take to get
%% it running, what its proper dependencies are ... or even what it does.
%% Given the density of hard-coded terms and complete lack of comments on
%% what's going on, it may not be worth keeping it al all.
%% On the plus side, it *does* compile.
-module(sc).
...
init(Nodes, _NumWorkers) ->
    ...
    riakc_pb_socket:put(C, riakc_obj:new(?BUCKET, <<?KEY:64/integer>>, term_to_binary([])),
        ?TIMEOUT),
...
modify(Key, Obj, Current, Thread, X) ->
    ...
    case untag(Obj) of
        ...
        _ ->
            ...
            %% C:put(NewObj).
            case result(
                riakc_pb_socket:put(C, NewObj, [return_body, {w, 1}, {timeout, ?TIMEOUT}])) of
                {error, siblings} ->
                    siblings;
                {error, failed} ->
                ...
                {error, timeout} ->
                    ...
                    %% %% ?debugFmt("Valid: ~p~n", [VL]),
                    %% Maybe = lists:any(fun(ValidObj) ->
                    %%                           ValidVal = value(ValidObj),
                    %%                           %% ?debugFmt("~p ??= ~p~n", [PriorVal, ValidVal]),
                    %%                           PriorVal =:= ValidVal
                    %%                   end, VL),
...
partition(As, Bs) ->
    ...
    case rpc:call(Node, riak_ensemble_manager, rget, [ensembles, []]) of
        Ensembles when is_list(Ensembles) ->
            ...
            %% ?debugFmt("Peers: ~p~nAs: ~p~nBs: ~p~n", [Peers, As, Bs]);
            %% ?debugFmt("AL: ~p~nBL: ~p~n", [AL, BL]),
            [rpc:call(Node, ets, insert, [sc, [{{drop, {A, B}}, true},
                {{drop, {B, A}}, true}]])
                || A <- AL,
                B <- BL],
            %% ?debugFmt("ETS: ~p~n", [ets:tab2list(sc)]),
...
put_once(Key, Current, Thread, X) ->
    ...
    case result(
        riakc_pb_socket:put(C, Obj, [return_body, if_none_match, {timeout, ?TIMEOUT}])) of
        {ok, ResultObj} ->
            {ok, ResultObj};
        {error, failed} ->
            {failed, untag(Current)};
...
%% may_fail(Prior, Current, #state{maybe_rewritten=MaybeRW}) ->
    ...
    ?debugFmt("Maybe/~p: ~p~n~p~n", [Maybe, MaybeRW, Prior]),
...
sc_test_() ->
    ...
    {timeout, Duration * 2,
        fun() ->
            io:put_chars(user, " Test disabled. Skipping ")
        end}.
            ...
            ?FORALL(Cmds, noshrink(more_commands(1, commands(?MODULE, State))),
                ?ALWAYS(Repetitions,
                    ...
                    begin
                        ...
                        %% ?debugVal(S#state.acked),
                        ?debugVal(Success),
                        C = ets:lookup_element(?ETS, {client, 1}, 2),
                        Pass2 = case result(
                            riakc_pb_socket:get(C, ?BUCKET, <<?KEY:64/integer>>, ?TIMEOUT)) of
                            {ok, Obj} ->
                            ...
                            {error, timeout} ->
                                ...
                                true;
                            {error, notfound} ->
                                ?debugMsg("Obj: notfound"),
                                (Success =:= [])
                            %% _ ->
                            %%     true
                        end,
                        Pass2 orelse ?debugMsg("Data loss!"),
...
wait_until_quorum(Retry, Ensemble, Node) ->
    case rpc:call(Node, riak_ensemble_manager, check_quorum, [Ensemble, 1000]) of
        true ->
            %% ?debugFmt("wait_for_quorum(~p): succeed", [Ensemble]),
            ok;
        _ when Retry =< 0 ->
            ?debugFmt("wait_for_quorum(~p): timeout", [Ensemble]),
            throw(failed);
        _ ->
            timer:sleep(100),
